...
class ArbolC4_5(Arbol):
    ...
    def __str__(self):
        def mostrar(t: ArbolC4_5, nivel: int):
            tab = '.' * 4
            indent = tab * nivel
    ...
    def construir(cls, X: np.ndarray, y: np.ndarray, 
        ...
        if len(np.unique(y)) == 1:
            return ArbolC4_5(y[0], es_hoja=True)
        ...
        if profundidad_max is not None and profundidad_actual >= profundidad_max:
            ...
            return ArbolC4_5(clase_mayoritaria, es_hoja=True)
        ...
        arbol = ArbolC4_5(mejor_atributo)
        ...
        for valor in np.unique(X[:, mejor_atributo]):
            ...
            if minimas_obs_h is not None and minimas_obs_h > len(sub_y):
                ...
                subarbol = ArbolC4_5(clase_mayoritaria, es_hoja=True)
            else:
